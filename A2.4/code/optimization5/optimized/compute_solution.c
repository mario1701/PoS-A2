...
int compute_solution(int nprocs, int myrank, const int max_iters, int nintci, int nintcf, int nextcf, int** lcc, double* bp,
  ...
  SCOREP_USER_REGION_BEGIN( handle1, "handle1 - Initialization of variables and reference residuals.",SCOREP_USER_REGION_TYPE_COMMON );
    ...
    while ( iter < max_iters ) {
      ...
      for ( nc = nintci; nc <= nintcf; nc += 4 ) {
	      d1 = _mm256_load_pd(&(direc1[nc]));
	      rv = _mm256_load_pd(&(resvec[nc]));
	      cg = _mm256_load_pd(&(cgup[nc]));
	      rv = _mm256_mul_pd(rv, cg);
	      d1 = _mm256_add_pd(d1, rv);
	      _mm256_store_pd(&(direc1[nc]), d1);
      }
      #else
      for ( nc = nintci; nc <= nintcf; nc++ ) {
	...
	nc = ext_access[i];
	  ...
	  SCOREP_USER_REGION_BEGIN( handle8, "handle8 - Computation phase2. occ computation",SCOREP_USER_REGION_TYPE_COMMON );
	    ...
	    __m256d d1, d2, o1, ax1, dx1;
		  
	    for ( nc = nintci; nc <= nintcf; nc += 4 ) {
		    d1 = _mm256_load_pd(&(direc1[nc]));
		    d2 = _mm256_load_pd(&(direc2[nc]));
		    ax1 = _mm256_load_pd(&(adxor1[nc]));
		    dx1 = _mm256_load_pd(&(dxor1[nc]));
		    o1 = _mm256_broadcast_sd(&oc1);

		    ax1 = _mm256_mul_pd(ax1, o1);
		    d2 = _mm256_sub_pd(d2, ax1);
		    _mm256_store_pd(&(direc2[nc]), d2);
		    
		    dx1 = _mm256_mul_pd(dx1, o1);
		    d1 = _mm256_sub_pd(d1, dx1);
		    _mm256_store_pd(&(direc1[nc]), d1);
	    }
	    #else
            for ( nc = nintci; nc <= nintcf; nc++ ) {
		...
		__m256d d1, d2, o1, ax1, dx1, o2, ax2, dx2;
		...
		for ( nc = nintci; nc <= nintcf; nc += 4 ) {
			...
			ax1 = _mm256_load_pd(&(adxor1[nc]));
			dx1 = _mm256_load_pd(&(dxor1[nc]));
			ax2 = _mm256_load_pd(&(adxor2[nc]));
			dx2 = _mm256_load_pd(&(dxor2[nc]));
			o1 = _mm256_broadcast_sd(&oc1);
			o2 = _mm256_broadcast_sd(&oc2);
			
			dx2 = _mm256_mul_pd(dx2, o2);
			dx1 = _mm256_mul_pd(dx1, o1);
			d1 = _mm256_sub_pd(d1, dx2);
			d1 = _mm256_sub_pd(d1, dx1);
			_mm256_store_pd(&(direc1[nc]), d1);
			
			ax2 = _mm256_mul_pd(ax2, o2);
			ax1 = _mm256_mul_pd(ax1, o1);
			d2 = _mm256_sub_pd(d2, ax2);
			d2 = _mm256_sub_pd(d2, ax1);
			_mm256_store_pd(&(direc2[nc]), d2);
		}
		#else
